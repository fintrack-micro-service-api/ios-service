pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'fintrack-ios-service'
        CONTAINER_NAME = 'fintrack-ios-service-container'
    ...
    stages {
        ...
        //                     -Dsonar.projectKey=fintrack-ios-service \
        //                     -Dsonar.sources=src \
        ...
        stage('Trigger ManifestUpdate') {
            steps {
                script {
                    try {
                        build job: 'ios-service-pipeline-2', parameters: [string(name: 'DOCKERTAG', value: env.BUILD_NUMBER)]
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
